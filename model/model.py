from .FeatureExtractionBranch import FrameExtraction
from .InformationFusingBranch import CrossFrameAttentionSubNet, MultiModeFusionBlock
from .ManifoldSamplingBranch import FlowEst
import torch
...
class network(nn.Module):
    def __init__(self, Cin, emb_dim, blocks, attn_layer):
        ...
        self.CFA = CrossFrameAttentionSubNet(blocks, attn_layer, emb_dim * 4, emb_dim * 8, (32, 32),
                                             8)  # cross-modal spatiotemporal information fusion branch
...
if __name__ == '__main__':
    time = torch.tensor([[[2024, 12, 30, 4, 0],
                         ...
                         [[2024, 12, 31, 4, 0],
                          [2024, 12, 31, 10, 0]]])
    n = network(4, 64, 4, 2).cuda()
    c = n(torch.randn(6, 4, 64, 64).cuda(), torch.randn(6, 4, 64, 64).cuda(), time, torch.randn(6, 2, 6, 64, 64).cuda())
