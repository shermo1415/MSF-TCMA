...
class network(nn.Module):
    def __init__(self, Cin, emb_dim, blocks, attn_layer):
        super().__init__()
        self.FE_field = FrameExtraction(Cin, emb_dim)
        self.FE_fy = FrameExtraction(6, emb_dim)
        # self.ModeFusion = MultiModeFusionBlock(emb_dim * 4, emb_dim * 8, (32, 32), attn_layer, 2)
        self.CFA = CrossFrameAttentionSubNet(blocks, attn_layer, emb_dim * 4, emb_dim * 8, (32, 32), 8)
        self.ES = FlowEst(blocks,Cin,emb_dim*4)
        # self.ES_fy = FlowEst(blocks,6,emb_dim*4,None, 2)
    def forward(self, i1, i2, time=None, fy=None, outputframes = 4):
        ...
        # f1_field = self.FE_field(torch.cat([i1, fy_1], 1))
        # f2_field = self.FE_field(torch.cat([i2, fy_2], 1))

        f1_fy = self.FE_fy(fy_1)
        f2_fy = self.FE_fy(fy_2)
        # f1_field = self.ModeFusion(f1_field, f1_fy)
        # f2_field = self.ModeFusion(f2_field, f2_fy)

        # print(f1_field.shape, f1_fy.shape, f2_field.shape, f2_fy.shape)
        f_fb, l_fb = self.CFA(f1_field, f2_field, time, f1_fy, f2_fy)
        out = self.ES(i1, i2, f_fb, l_fb, outputframes)
        # outfy = self.ES_fy(fy_1, fy_2, f_fb)
        
        # print(outputframes, out.shape)
        return out
